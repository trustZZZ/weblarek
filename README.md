...
Поля интерфейса
`id: number` - уникальный индентификатор товара.
...
`interface IBuyer` - интерфейс покупателя, соответствующий данным, которые приходят с сервера.
  `payment: 'card' | 'cash'` - тип оплаты (онлайн или наличными).
...
`productDetailed: IProduct` - хранит товар для подробного отображения.
...
Методы класса:
 ...
 `getProductById(id: number): IProduct` - возвращение товара по id.
 `saveProductDetailed(product: IProduct): void` - сохранение товара для подробного отображения.
 `getProductDetailed(): IProduct` - получение товара для подробного отображения.
...
Поля класса:
`wishedProducts: IProduct[]` - массив товаров, выбранных покупателем для покупки.
...
`payment: 'online' | 'offline'` - вид оплаты.
...
`saveData<T>(...data: Array<T>[]): void` - сохранение данных в модели.
`getData(): IBuyer` - получение всех данных покупателя.
`clearData(): void` - очистка данных покупателя.
`validateData(data): Object` - валидация данных.
...
`async post(data: Object): Promise<Object>` - отправка данных на сервер.
`async get(uri: string): Promise<IResponse>` - получение с сервера объекта с массивом товаров.
